...
int init()
{
    ...
    DEBUG_printf("BytesPerLine = 0x%x\tLinBytesPerScanLine=0x%x\n", BOOTINF.CurVBEInfo->BytesPerLine, BOOTINF.CurVBEInfo->LinBytesPerScanLine);
#endif
    // _VMM_map(BOOTINF.PModeINT, 0xFF8000, FALSE, TRUE);
    // _VMM_map((((uint32_t)(BOOTINF.PModeINT)) + 0x1000), 0xFF9000, FALSE, TRUE);
    // if(!BOOTINF.PModeINT->SetDispStartOffset)
        // return -1;
    // VBE_PAGE0 = 0;
    ...
    // SecondFrameBuffer = (void*) (0xF00000 + ((Width*Height*(Depth >> 3))));
    VBE_LFB = 0xF00000;
    // VBE_LFB1 = (0xF00000 + ((Width*Height*(Depth >> 3))));
    // VBE_FUNCTION = (0xFF8000 + (((uint32_t)(BOOTINF.PModeINT)) % 0x1000) + ((uint32_t)(BOOTINF.PModeINT->SetDispStartOffset)));

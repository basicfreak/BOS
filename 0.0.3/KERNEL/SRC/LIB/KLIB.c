...
void strgetseg(volatile char* ret, const char* input, const char seperator, int position)
{
	int x = 0, pos = 0;
	while(*input && pos!=position)
		if(*input++ == seperator)
			pos++;
	if(pos==position)
		while(*input && *input != seperator)
			ret[x++] = *input++;
}

int strcountchar(const char* input, const char lookfor)
{
	int ret = 0;
	while(*input)
		if(*input++ == lookfor)
...
char **strexp(const char* input, const char seperator)
{
	char** ret = (char**) calloc(( (uint32_t)sizeof(char*) * (uint32_t)(strcountchar(input, seperator) + 2) ));
	int x = 0;
	for(; x < (strcountchar(input, seperator) + 1); x++) {
		ret[x] = (char*) calloc(128);
		strgetseg(ret[x], input, seperator, x);
	}
	x++;
	ret[x] = 0;
	return ret;
...
void freestrexp(char** sexp)
{
	int x = 0;
	while(sexp[x])
		free(sexp[x++], 128);
	free(sexp, ((uint32_t)(x + 1) * (uint32_t)sizeof(char*)));
}
...
int stringf(char *str, const char *format, va_list ap)
{
	...
	void		DEBUG_putch				(const char charactor)
	{
		...
		if (y >= maxY) {
			memcpy(&vidmem[0], &vidmem[160], (160*24));
			memset(&vidmem[160*24], 0, 160);
			y = (maxY - 1);
		}*/
	}

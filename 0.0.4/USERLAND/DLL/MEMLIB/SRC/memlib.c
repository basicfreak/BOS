		...
		if(((HStack_p)_HEAPSTACK)[y].Length >= (Len * _PAGESIZE)) {
			...
			for(int x = _HEAPFREEENTS; x < _HEAPALLOCENTS; x++) {
				if(!((HStack_p)_HEAPSTACK)[x].Length) {
					((HStack_p)_HEAPSTACK)[x].Start = ret;
					((HStack_p)_HEAPSTACK)[x].Length = (Len * _PAGESIZE);
					((HStack_p)_HEAPSTACK)[y].Start = (void*) ((uint32_t)(ret) + (Len * _PAGESIZE));
					((HStack_p)_HEAPSTACK)[y].Length -= (Len * _PAGESIZE);
					while(Len)
						_VMM_alloc((((uint32_t)ret) + (--Len * _PAGESIZE)), TRUE);
					return ret;
				}
			}
		}
...
void* calloc(uint32_t Length)
{
	...
	Bochs_printf("Page Allocated start = 0x%x\tlength = 0x%x\n", (uint32_t)ret, Length);
	if(ret)
		memset(ret, 0, Length);
	return ret;
}

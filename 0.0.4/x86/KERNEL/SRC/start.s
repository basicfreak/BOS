...
HigherHalf:
	...
	rep movsd
	; Copy MODLIST
	mov esi, DWORD [BOOTINF.ModList]
	mov edi, MODLIST
	mov DWORD [BOOTINF.ModList], edi
	movzx ecx, BYTE [BOOTINF.ModCount]
	shl ecx, 1
	rep movsd
	; Copy VBE INFO if present
	mov eax, DWORD [BOOTINF.CurVBEInfo]
	test eax, eax
	jz kmain
	mov esi, eax
	mov edi, VBEINFO
	mov DWORD [BOOTINF.CurVBEInfo], edi
	mov ecx, 32
	rep movsd
	jmp kmain
...
SECTION .bss ; Reserved Area, to be allocated upon loading ELF
align 0x00001000
